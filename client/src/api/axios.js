...
axiosInstance.interceptors.response.use(
  (response) => {
    // Запрос прошёл — скрываем toast пробуждения если был
  ...
  async (error) => {
    ...
    if (error.response) {
      ...
      if (status === 401) {
        ...
        // Правильный редирект с учётом BASE_URL (GitHub Pages)
    ...
    // Нет ответа = сетевая ошибка (сервер спит / ERR_INTERNET_DISCONNECTED)
    // Фоновые polling-запросы (_noRetry) — тихо проваливаются без retry и toast
    if (config._noRetry) {
      return Promise.reject({ message: 'Сервер не отвечает', status: 0 });
    }

    // Ретраим только безопасные запросы (GET) или первую попытку для любого метода
